...
| | CREDITS |
|---|---|
| Весь покупаемый каталог (9 предметов) | **1460** |
...
чистый уровень. План расширения предупреждал об этом и назвал лечение —
**предметы, которые нельзя купить, только заработать**
(`unlockCondition: { kind: 'stars' }`). Механизм построен, но пользуется им
пока один предмет — `reference` на 90 звёзд. Решение о том, добавлять ли
такие пороги остальным премиум-предметам или менять `STAR_CREDITS`,
за владельцем: оно меняет зафиксированный объём косметики в `TODO.md`.
