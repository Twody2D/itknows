...
## Проверка по требованиям площадки (2026-09-21)

Источник: `yandex.ru/dev/games/doc/ru/concepts/requirements`. Проверено по коду
и замерами на прод-сборке; найдено шесть расхождений, все закрыты.

| Пункт | Было | Стало |
|---|---|---|
| 1.3 «При потере фокуса звук останавливается» | `visibilitychange` гасил звук для свёрнутой вкладки, но зажатая клавиша оставалась зажатой навсегда (замерено: 87 px ходьбы за 0.7 с после blur) | `InputState` отпускает всё на `blur`/`pagehide`/`pointercancel`/скрытой странице (замерено: 2.3 px торможения, `right: false`) |
| 1.6.1.1 полный экран на мобильных | `requestFullscreen` не вызывался нигде | `utils/input/fullscreen.ts` — один раз, по первому касанию, только на coarse-указателе |
| 1.6.2.2 соотношение сторон ≤ 2:1 | 620×270 = 2.30; на 1920×937 выходило 2.05 | `MAX_VIRTUAL_WIDTH = VIRTUAL_HEIGHT * 2` — замерено 2.000 на шести вьюпортах |
| 2.14 автоопределение языка | не было; `ru` для всех, и выбор игрока не сохранялся | `environment.i18n.lang` → `LocaleState.applyDetected`, выбор игрока сильнее и пишется в `itknows.locale.v1` |
| 4.5.2 награда не влияет на возможность продолжить | continue в Daily Challenge давался за ролик | continue бесплатен, один в день; rewarded остался только за CREDITS |
| 4.7 пауза звука и игры на рекламе | `onOpen` не был подписан ни у одного ролика | `onAdBreak` — `GameplayAPI.stop/start` в фасаде, звук/Phaser/ввод в `main.ts` |

Важное побочное следствие потолка 540: декор правой колонки главного меню
(серверная стойка и строка SYSTEM) был расставлен под 620 и стал бы
недосягаем на любой ширине — то есть мёртвым кодом. Оба привязаны к
правому краю и проверяются тестом `tests/menu-layout.test.ts`.
