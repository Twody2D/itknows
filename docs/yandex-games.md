...
- **Leaderboards** (`ysdk.leaderboards.setScore/getEntries/getPlayerEntry`)
  — `setScore`/`getPlayerEntry` требуют авторизации; `getEntries` — нет.
  Утверждённое раньше решение (см. TODO.md, «Решённые вопросы» #4):
  лидерборды только по каноничной вариации уровня и Daily Challenge, не по
  произвольным `variantId`.
